...
std::function<void(uint16_t, uint16_t, uint64_t, std::vector<char>)> callbackReceive = nullptr;
...
bool HelloWorldSubscriber::init(
    ...
    rqos = eprosima::fastdds::dds::DATAREADER_QOS_DEFAULT;
    rqos.reliability().kind = eprosima::fastdds::dds::RELIABLE_RELIABILITY_QOS;
    rqos.reliability().max_blocking_time = {10, 0};
...
void HelloWorldSubscriber::SubListener::on_data_available(
    ...
    SampleInfo info;
    HelloWorld h;
    if (reader->take_next_sample(&h, &info) == ReturnCode_t::RETCODE_OK) {
        if (info.instance_state == ALIVE_INSTANCE_STATE) {
            samples_++;
            // Print your structure data here.

            if (callbackReceive) {
                callbackReceive(h.from(), h.to(), h.index(), h.message());
            }
        }
    }
}

void HelloWorldSubscriber::run(const std::function<void(uint16_t, uint16_t, uint64_t, std::vector<char>)> &callback) {
    callbackReceive = callback;
}
